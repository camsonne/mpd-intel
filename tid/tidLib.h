...
#define TID_TSINPUT_ALL       (0x3F)

/* Define default Interrupt vector and level */
#define TID_INT_VEC      0xc7
#define TID_INT_LEVEL    5
...
#define TID_ENABLEBITS_BUSY_DISABLE_FIFO (1<<15)
...
#define TID_RESET_BLOCK_READOUT (1<<17)
...
int  tidSoftTrig(unsigned int nevents, unsigned int period_inc, int range);
int  tidSetRandomTrigger(int setting);
int  tidSetRandomTrigger2(int setting);
...
int  tidGetClockSource(int pflag);
int  tidVmeTrigger1();
int  tidVmeTrigger2();
void tidSetFiberDelay(unsigned int number, unsigned int offset);
int  tidAddSlave(unsigned int fiber);
int  tidSetTriggerHoldoff(int rule, unsigned int value);
int  tidGetTriggerHoldoff(int rule);
void tidResetBlockReadout();
int  tidDisableA32();
unsigned int tidBlockStatus(int fiber, int pflag);
#endif /* TIDLIB_H */
