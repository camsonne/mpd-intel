...
/*   stat = vmeBusToLocalAdrs(0x39,(char *)tAddr,(char **)&laddr); */
  ...
  if(fpgaUserCode>0)
    ...
    {
      printf("  ERROR: Invalid firmware 0x%08x\n",fpgaUserCode);
      return ERROR;
    }
...
tidStatus()
{
  ...
  TIDUNLOCK;
  fmVersion = tidGetFirmwareVersion();
...
#endif
  printf("  User ID: 0x%x \tFirmware (version - revision): 0x%X - 0x%03X\n",
	 (fmVersion&0xFFFF0000)>>16, (fmVersion&0xF000)>>12, fmVersion&0xFFF);
  printf("--------------------------------------------------------------------------------\n");
  ...
  if(tidTriggerSource)
    {
      ...
      if(tidTriggerSource & TID_DATASETUP_TRIGSRC_P0)
	printf("   P0 Input\n");
      if(tidTriggerSource & TID_DATASETUP_TRIGSRC_HFBR1)
	printf("   HFBR #1 Input\n");
      if(tidTriggerSource & TID_DATASETUP_TRIGSRC_LOOPBACK)
	printf("   TI Master Loopback\n");
      if(tidTriggerSource & TID_DATASETUP_TRIGSRC_FPTRG)
	printf("   Front Panel TRG\n");
      if(tidTriggerSource & TID_DATASETUP_TRIGSRC_VME)
	printf("   VME Command\n");
      if(tidTriggerSource & TID_DATASETUP_TRIGSRC_TSINPUTS)
	printf("   Front Panel TS Inputs\n");
      if(tidTriggerSource & TID_DATASETUP_TRIGSRC_TSrev2)
	printf("   Trigger Supervisor (rev2)\n");
      if(tidTriggerSource & TID_DATASETUP_TRIGSRC_RND)
  ...
  if(enableBits & TID_ENABLEBITS_BUSY_ENABLE_HFBR8)
    printf("   HFBR #8\n");
  if(enableBits & TID_ENABLEBITS_BUSY_DISABLE_FIFO)
    printf("   FIFO BUSY DISABLED\n");
...
tidSetTriggerSource(int trig)
{
  ...
  printf("%s: INFO: tidTriggerSource = 0x%x\n",__FUNCTION__,tidTriggerSource);
...
tidDisableTriggerSource(int trig)
{
  ...
  TIDLOCK;
  if(trig) /* Assume TS mode... disable all external inputs */
    {
      vmeWrite32(&TIDp->dataSetup,
		 vmeRead32(&TIDp->dataSetup) & ~(
						 TID_DATASETUP_TRIGSRC_P0 |
						 TID_DATASETUP_TRIGSRC_FPTRG |
						 TID_DATASETUP_TRIGSRC_VME |
						 TID_DATASETUP_TRIGSRC_TSINPUTS |
						 TID_DATASETUP_TRIGSRC_RND
						 )
		 );
    }
  else
 ...
 *  period_inc  - period multiplier, depends on range (0-0xFFFF)
 *  range       - small (0) or large (1)
 *     For small, the period range:
 *            minimum of 160ns, increments of 80ns up to 163.76us
 *     In large, the period range:
 *            minimum of 160ns, increments of 81.92us up to 167.69ms
 *
 * RETURNS: OK if successful, ERROR otherwise
 *
 */
...
int
tidSoftTrig(unsigned int nevents, unsigned int period_inc, int range)
{
  ...
  if(period_inc>0xFFFF)
    {
      logMsg("\ntidSoftTrig: ERROR: period_inc must be less than %d ns\n",
	     periodMax,2,3,4,5,6);
      return ERROR;
    }
  if( (range!=0) && (range!=1) )
    {
      logMsg("\ntidSoftTrig: ERROR: range must be 0 or 1\n",
	     periodMax,2,3,4,5,6);
      return ERROR;
    }

  if(range==0)
    time = 80+160*period_inc;
  if(range==1)
    time = 80+160*period_inc*1024;

  logMsg("\ntidSoftTrig: INFO: Setting software trigger for %d nevents with period of %d\n",
	 nevents,time,3,4,5,6);

  TIDLOCK;
  vmeWrite32(&TIDp->softTrig,
	     (range<<31)| (period_inc<<16) | (nevents));
  TIDUNLOCK;
...
int
tidSetRandomTrigger(int setting)
{
  double rate;
  ...
  setting |= (1<<3);  /* Set the enable bit */

  printf("%s: Enabling random trigger at rate (kHz) = %.2f\n",
	 __FUNCTION__,rate);

  TIDLOCK;
  vmeWrite32(&TIDp->randomTrig, (setting | (setting<<4)) );
  TIDUNLOCK;
  return OK;
}

int
tidSetRandomTrigger2(int setting)
{
  double rate;
  ...
  TIDLOCK;
  vmeWrite32(&TIDp->randomTrig, (setting | (setting<<4))<<8 );
  TIDUNLOCK;
  return OK;
}
...
tidClockReset()
{
  ...
  if(tidMaster!=1)
    {
      printf("%s: ERROR: TID is not the Master.  No Clock Reset.\n", __FUNCTION__);
      return;
    }
  ...
  taskDelay(2);
  /* Disable Loopback Sync before Clock Reset issued */
  vmeWrite32(&TIDp->dataSetup,
	     vmeRead32(&TIDp->dataSetup) & ~TID_DATASETUP_SYNCSRC_MASK);
  taskDelay(2);
  vmeWrite32(&TIDp->vmeSyncLoad,TID_250_CLOCK_RESET); 
  taskDelay(2);
  TIDUNLOCK;

  /* Enable Loopback as sync source */
  tidSetSyncSource(TID_SYNC_LOOPBACK);
...
tidIntPoll()
{
  ...
  if(TIDp == NULL) 
    {
      logMsg("tidIntPoll: ERROR: TID not initialized\n",1,2,3,4,5,6);
      return ERROR;
    }
...
tidIntDisconnect()
{
  ...
  if(tidIntRunning) 
    {
      logMsg("tidIntDisconnect: ERROR: TID is Enabled - Call tidIntDisable() first\n",
	     1,2,3,4,5,6);
      return ERROR;
    }
...
{
  if(TIDp == NULL) {
    logMsg("tidIntAck: ERROR: TIR not initialized\n",0,0,0,0,0,0);
    return;
  }
...
#endif
      vmeWrite32(&TIDp->enableBits,
	       vmeRead32(&TIDp->enableBits) | (1<<13) );
      break;
...
tidBReady()
{
  ...
  if(TIDp == NULL) 
    {
      logMsg("tidBReady: ERROR: TID not initialized\n",1,2,3,4,5,6);
      return 0;
    }
...
tidGetClockSource(int pflag)
{
  ...
  TIDLOCK;
  rval = (vmeRead32(&TIDp->intSetup) & (TID_INTSETUP_CLK_SOURCE_MASK))>>12;
  TIDUNLOCK;
  ...
  if(pflag)
	...
	{
	case 0:
	  printf("%s: TI Using Internal Clock\n",__FUNCTION__);
	  break;
	case 1:
	  printf("%s: TI Using HFBR 5 Clock\n",__FUNCTION__);
	  break;
	case 2:
	  printf("%s: TI Using HFBR 1 Clock\n",__FUNCTION__);
	  break;
	case 3:
	  printf("%s: TI Using Front Panel Clock\n",__FUNCTION__);
	  break;
	default:
	  printf("%s: TI Using UNDEFINED CLOCK!\n",__FUNCTION__);
	}
    }
...
/*   vmeWrite32(&TIDp->triggerCmdCode, 0x0180); */
  vmeWrite32(&TIDp->softTrig2,0x1| (1<<16));
  TIDUNLOCK;
  return OK;
 ...
 *
 *    value - the specified time period (in steps of 16ns)
 *
 *   RETURNS: OK if successful, otherwise ERROR.
 *
 */
...
int
tidSetTriggerHoldoff(int rule, unsigned int value)
{
  unsigned int wval=0, rval=0, oldvalue=0;
  ...
  if( (rule==1) && (value>31) )
    {
      printf("%s: ERROR: Invalid value for rule=1 (%d). Must be less than 31.\n",
	     __FUNCTION__,value);
      return ERROR;
    }
  if( (rule==2) && (value>127) )
    {
      printf("%s: ERROR: Invalid value for rule=2 (%d). Must be less than 127.\n",
	     __FUNCTION__,value);
      return ERROR;
    }
  ...
  TIDLOCK;
  rval = vmeRead32(&TIDp->trigger_holdoff_read);
  TIDUNLOCK;
  
  if(rule==1)
    {
      oldvalue = (rval & 0x7F000000)>>24; /* Old rule 2 value */
      wval     = (oldvalue<<8) | value;
    }
  else
    {
      oldvalue = (rval & 0x001F0000)>>16; /* Old rule 1 value */
      wval     = oldvalue | (value<<8);
    }

  TIDLOCK;
  vmeWrite32(&TIDp->trigger_holdoff,wval);
  TIDUNLOCK;
...
tidGetTriggerHoldoff(int rule)
{
  ...
  if(rule==1)
    rval = (rval & 0x001F0000)>>16;
  
  if(rule==2)
    rval = (rval & 0x7F000000)>>24;
...
tidResetBlockReadout()
{
  ...
  if(TIDp == NULL) 
    {
      logMsg("tidResetBlockReadout: ERROR: TID not initialized\n",1,2,3,4,5,6);
      return;
    }
 
  TIDLOCK;
  vmeWrite32(&TIDp->reset,TID_RESET_BLOCK_READOUT);
  TIDUNLOCK;
...
tidDisableA32()
{
  ...
  TIDLOCK;
  vmeWrite32(&TIDp->adr32,0x0);
  vmeWrite32(&TIDp->enableBits, 
	     vmeRead32(&TIDp->enableBits) |
	     TID_ENABLEBITS_BUSY_DISABLE_FIFO);
  TIDUNLOCK;
...
unsigned int
tidBlockStatus(int fiber, int pflag)
{
  unsigned int rval=0;
  char name[50];
  ...
  switch(fiber)
    {
    case 0:
      rval = (vmeRead32(&TIDp->blockStatus[4]) & 0xFFFF0000)>>16;
      break;
    ...
    case 5:
    case 7:
      rval = (vmeRead32(&TIDp->blockStatus[(fiber-1)/2]) & 0xFFFF);
      break;
    ...
    case 6:
    case 8: 
      rval = ( vmeRead32(&TIDp->blockStatus[(fiber/2)-1]) & 0xFFFF0000 )>>16;
      break;
    }

  if(pflag)
    {
      nblocksReady   = rval & TID_BLOCKSTATUS_NBLOCKS_READY0;
      nblocksNeedAck = (rval & TID_BLOCKSTATUS_NBLOCKS_NEEDACK0)>>8;
