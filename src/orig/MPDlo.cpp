...
{
  std::cout<<"\n\n LOOK HERE\n"<<slot<<" "<<rev_id<<" "<<timec<<std::endl; 
  MPD_DBG("\n\n LOOK HERE\n %d %d %d",slot,rev_id,timec); 
...
void MPDlo::BOARD_Init(void)
{
  ...
  // ** I2C **
  if( I2C_Init() != BUS_OK )
    cout << __FUNCTION__ << ": I2C_Init() failed" << endl;
  ...
  if( I2C_ApvReset() != BUS_OK )
    cout << __FUNCTION__ << ": I2C_ApvReset() failed" << endl;

  // ** APV **
  for(i=0; i<GetApvCount(); i++) {
    if( APV_Config(i) != BUS_OK )
      cout << __FUNCTION__ << ": Failure configuring APV " 
	   << dec << fApv[i].i2c << " in Board " << fSlot << " <<<=== ***" << endl; // Adc[i]->fApv[j].i2cAddrScan 
  }

  // ** ADCs **
  for (i=0;i<2;i++) {
    if( ADS5281_SetParameters(i) != BUS_OK )
      cout << __FUNCTION__ << ": VME Slot " << dec << fSlot << " ADS5281_SetParameters("<<i<<") failed" << endl;
    ...
    if( ADS5281_SetGain(i, 
      ...
      MPD_WRN("VME Slot %d, ASetGain group %d failed\n",fSlot,i);
    //    }
...
int MPDlo::I2C_Init(void)
{
  ...
  */
  ...
  MPD_DBG("First VME call next Write(0x%x, 0x%x)\n",addr, data); 
  if( (success = BUS_Write(addr, &data)) != BUS_OK )
    {
      printf("I2C_Init(): Write control register(1) failed\n");
      return success;
    }
  MPD_DBG("First VME call done Write(0x%x, 0x%x) -> %d\n",addr, data, success); 
  ...
  if( (success = BUS_Write(addr, &data)) != BUS_OK )
    {
      printf("I2C_Init(): Write low prescaler failed\n");
      return success;
    }
  
  BUS_Read(addr, &rdata);
  MPD_MSG(" Prescale Low W=%d R=%d\n", data, rdata);

  addr = I2CcontrollerOffset + 4;		/* Prescaler high (RW) */
  data = (ispeed>>8) & 0xff;  
  if( (success = BUS_Write(addr, &data)) != BUS_OK )
    {
      printf("I2C_Init(): Write high prescaler failed\n");
      return success;
    }
  BUS_Read(addr, &rdata);
  MPD_MSG(" Prescale High W=%d R=%d\n", data, rdata);

  printf("I2C_Init(): speed prescale = %d, (period = %f us, frequency = %f kHz)\n",ispeed, ispeed/10., 10000./ispeed);
  
  addr = I2CcontrollerOffset + 8;	/* Control Register (RW) */
  data = 0x80;	/* Enable I2C Core and disable interrupts */
  if( (success = BUS_Write(addr, &data)) != BUS_OK )
    printf("I2C_Init(): Write control register(2) failed\n");
  return success;
...
int MPDlo::I2C_ByteWrite(unsigned char dev_addr, unsigned char int_addr, int ndata, unsigned char *data)
{
  ...
  MPD_DUM("I2C Send Byte 1 to 0x%x return %d\n",dev_addr, success);
  ...
  MPD_DUM("I2C Send Byte 0 to 0x%x return %d\n",dev_addr, success);
  //  usleep(300);
  for(i=0; i<ndata; i++)
...
int MPDlo::APV_Read(unsigned char apv_addr, unsigned char reg_addr, unsigned char *val)
{
  ...
  MPD_MSG("Set / Get = 0x%x 0x%x (Read returned %d)\n",*val,rval,success);
...
int MPDlo::APV_Config(int apv_index)
{
  ...
  cout << __FUNCTION__ << ": i2c = " << (int) apv_addr << " at adc (fifo)= " << v2a(apv_index) << endl;
  for (i=0;i<18;i++) {
    switch (i) {
    case 0:
      reg_addr = mode_addr;
...
int MPDlo::TRIG_Enable() {
  ...
  data = sync_period << 16 | GetApvEnableMask();
...
int MPDlo::ADS5281_SetParameters(int adc)	/* adc == 0, 1 */
{
  ...
  if ((success = ADS5281_Set(adc, 0x428020)) != BUS_OK) { return success; } // 0x428020
  usleep(MPD_ADC_USLEEP);
...
int MPDlo::HISTO_Clear(int ch)	/* ch == 0, 15 */
{
  uint32_t addr;
  uint32_t data[4096];
  int success, i, j,ntimes;
...
//			   int &n_events)   // number of events
{
  ...
  MPD_DBG("%s: Read apv = %d, wrec = %d, success = %d\n","DEB", channel, wrec, success);
#endif

  if( wrec != size ) return BUS_GENERIC_ERROR;
...
int MPDlo::FIFO_ReadSingle(int channel, int blen, uint32_t *event, int &nread)
{
  ...
  size = (nwords > blen) ? blen : nwords; // cannot exceed buffer size
...
bool MPDlo::FIFO_ReadAll(int &timeout, int &global_fifo_error) {
  ...
  if (fReadDone == false) { // at least one MPD FIFO needs to be read
	...
	err = FIFO_ReadSingle(v2a(k), ApvGetBufferPWrite(k), nread, 1); 
