...
class MPDlo
{
 private:
  ...
  int BUS_Write(uint32_t Offset, void *Data) {
    int *dummy;
    dummy = (int *) Data;
    MPD_DUM(" Write data 0x%x on bus at address 0x%x\n", *dummy, fBaseAddr+Offset);
 ...
 public:
  ...
  /* From Histo.cpp */
  int HISTO_Clear(int ch);	/* ch == 0, 15 */
  ...
  void ApvBufferAlloc(int ia) {
    fApv[ia].fBufSize = 3*fApv[ia].fNumberSample*(EVENT_SIZE+2); // at least 3 event large
  ...
  int ApvGetBufferAvailable(int ia) {
    return fApv[ia].fBufSize - fApv[ia].fBi1;
  };
  ...
  int GetAdcClockPhase(int adc) { return fAdcClockPhase[adc]; };
  ...
  uint32_t GetFpgaRevision(void) {return FpgaRevision; };
  void SetFpgaRevision(uint32_t r) { FpgaRevision = r; };
